    ...
    try:
        async with streamable_http_client(server_url) as client:
            async with ClientSession(*client) as session:
                ...
                print_section("Test 3: Get food by ID (get-food-by-id)")
                ...
                # Test 4: Search for foods with barcode
                print_section("Test 4: Get food by barcode (get-food-by-ean13)")
                print_info("Searching for foods with barcodes...")
                ...
                barcode_foods = json.loads(result_barcode_search.content[0].text)
                barcode_found = None
                ...
                if barcode_found:
                    print_info(f"Found barcode: {barcode_found}")
                    ...
                    barcode_food = json.loads(result4.content[0].text)
                    if barcode_food:
                        print_success("Retrieved food by barcode")
                        print(f"\n  \033[1m{barcode_food['name']}\033[0m")
                        print(f"  Barcode: {barcode_food['ean_13']}")
                        print(f"  Type: {barcode_food.get('type', 'N/A')}")
                else:
                    print_info("No foods with valid barcodes found in sample")

                # Test 5: Pagination test
                print_section("Test 5: Pagination Test")
                ...
                page1_foods = json.loads(page1.content[0].text)
                page2_foods = json.loads(page2.content[0].text)
...
if __name__ == "__main__":
    ...
    print("  python3 -m mcp_opennutrition --transport streamable-http\n")
